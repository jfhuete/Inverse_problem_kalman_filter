...
% SNR of the problem (100 -> ideal; otherwise -> real)
SNR_vs = [100 50];     % SNR of meassured equation in dB
...
Hs(1) = struct('Type','Identity','H',H_Identity);
% Hs() = struct('Type','Adjancency','H',H_Adjancency);
% Hs() = struct('Type','Neighbor Gaussian step 2','H',H_GaussSteps2);
% Hs() = struct('Type','Neighbor Gaussian step 5','H',H_GaussSteps5);
% Hs() = struct('Type','Distance Gaussian 2 Neighbor','H',H_GaussDistance2);
% Hs() = struct('Type','Distance Gaussian 5 Neighbor','H',H_GaussDistance5);
...
% Vertex
vertexs=[20, 1000];               % Vertex that you want to plot
